...
struct omni_context {
    ...
    // 设置为 0 表示无限制
    int max_new_speak_tokens_per_chunk = 200;

    // min_speak_tokens: 每次说话至少生成多少个有效 token 后才允许 <|listen|>
    // 在前 min_speak_tokens 个 token 内，<|listen|> 的 logit 被设为 -inf
    // 防止模型在只说了几个字后就切换到听模式
    int min_speak_tokens = 100;

    // 当前说话轮次已生成的有效 token 数（在 stream_decode 中更新）
    int current_speak_token_count = 0;
...
llama_token sample_tts_token(struct common_sampler *          smpl,
                             ...
                             common_params *                  params,
                             int *                            n_past_tts,
                             const std::vector<llama_token> * all_generated_tokens   = nullptr,
                             const std::vector<llama_token> * chunk_generated_tokens = nullptr,
                             int                              token_index_in_chunk   = 0,
