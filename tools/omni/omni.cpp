...
static bool is_end_token(struct omni_context * ctx, llama_token token) {
    ...
    if (ctx->duplex_mode) {
        // 双工模式:
        // [修复短回复] 不再在 chunk_eos/chunk_tts_eos 处 break
        // 让 LLM 在一次 stream_decode 中持续生成，直到真正的轮次结束 token。
        // chunk_eos 仅用于 TTS 分段流式播放，不应中断 LLM 生成。
...
static const char * sample_with_hidden_and_token(struct common_sampler * smpl,
    ...
    if (ctx_omni->duplex_mode) {
        if (logits != nullptr) {
            // 1. 🔧 [修复短回复] 在前 min_speak_tokens 个 token 内，禁止所有结束 token
            // 包括: <|listen|>, <|turn_eos|>, <|tts_eos|>, </s> (EOS)
            // 这样模型必须至少生成足够多的有效 token 才能结束说话
            ...
            if (in_min_speak_window) {
                ...
                // 超过最小阈值后，正常应用 listen_prob_scale
                if (ctx_omni->special_token_listen >= 0) {
...
struct omni_context * omni_init(struct common_params * params,
    ...
    if (duplex_mode) {
        ...
        ctx_omni->audio_assistant_prompt =
            "<|audio_end|>"
            "你的任务是用这种声音模式来当一个助手。请认真、高质量地回复用户的问题。请用高自然度的方式和用户聊天。你是由"
            "面壁智能开发的人工智能助手：面壁小钢炮。<|im_end|>\n<|im_start|>user\n";
        ...
        ctx_omni->omni_assistant_prompt =
            "<|audio_end|>"
            "你的任务是用这种声音模式来当一个助手。请认真、高质量地回复用户的问题。请用高自然度的方式和用户聊天。<|im_"
            "end|>\n<|im_start|>user\n";
    }
    ...
    if (use_tts && !params->tts_model.empty()) {
        ...
        tts_sampling.top_p =
            0.85f;  // 🔧 [与 Python 对齐] TTSSamplingParams.top_p=0.85             // 🔧 [与 Python streaming 对齐] top_p=0.8
        ...
        if (!queue.empty()) {
            ...
            for (int il = 0; il < (int) llm_embeds.size(); ++il) {
                ...
                if (embeds->vision_embed.size() > 0) {
                    ...
                    print_with_timestamp(
                        "Omni模式: %d vision chunks (%d tokens each), %d audio tokens, has_slices=%d\n", n_chunks,
...
static bool generate_audio_tokens_local(struct omni_context *      ctx_omni,
    ...
    const int min_new_tokens =
        ctx_omni->duplex_mode ? (is_end_of_turn ? 0 : 26) : 100;  // 🔧 单工模式：至少生成 100 个 tokens 防止过早 EOS
            ...
            if (ctx_omni->token2wav_session->feed_window(window, is_last_window, chunk_wav)) {
                ...
                if (!chunk_wav.empty()) {
                    ...
                    if (f_wav) {
                        ...
                        print_with_timestamp(
                            "T2W线程: wav_%d.wav | %.2fs audio | %.1fms inference | RTF=%.2f | t=%lldms\n",
